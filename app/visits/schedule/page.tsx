...
export default function ScheduleVisitPage() {
  ...
  const fetchOfficers = useCallback(async () => {
    const response = await apiClient.get('/officers') as any;
    if (response.success) {
  ...
  const fetchVisits = useCallback(() => {
    if (!formData.seniorCitizenId) return Promise.resolve({ data: { visits: [] } });
    return apiClient.getVisits({ citizenId: formData.seniorCitizenId, limit: 20 });
  }, [formData.seniorCitizenId]);

  const { data: citizensData } = useApiQuery<{ data: { citizens: CitizenOption[] } }>(fetchCitizens, {
    refetchOnMount: true,
    enabled: !!user && ['ADMIN', 'SUPER_ADMIN', 'OFFICER'].includes(user.role)
  });

  const { data: officersData } = useApiQuery<{ data: OfficerOption[] }>(fetchOfficers, {
    refetchOnMount: true,
    enabled: !!user && ['ADMIN', 'SUPER_ADMIN', 'OFFICER'].includes(user.role)
  });

  const { data: visitsData, loading: visitsLoading, refetch: refetchVisits } = useApiQuery<{ data: { visits: Visit[] } }>(fetchVisits, {
    refetchOnMount: true,
