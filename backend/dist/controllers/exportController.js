...
class ExportController {
     ...
     */
    static async exportCitizens(req, res) {
        try {
            const { districtId, policeStationId, beatId, status, vulnerabilityLevel, search } = req.query;
            const where = {};
            if (districtId)
                where.Beat = { PoliceStation: { districtId: String(districtId) } };
            if (policeStationId)
                where.Beat = { ...where.Beat, policeStationId: String(policeStationId) };
            if (beatId)
                where.beatId = String(beatId);
            if (status)
                where.status = String(status);
            if (vulnerabilityLevel)
                where.vulnerabilityLevel = String(vulnerabilityLevel);
            if (search) {
                where.OR = [
            ...
            const csv = await exportService_1.ExportService.generateCitizensCSV(citizens);
     ...
     */
    static async exportVisits(req, res) {
        try {
            ...
            const buffer = await exportService_1.ExportService.generateVisitsExcel(visits);
    ...
    static async generateReport(req, res) {
        try {
            ...
            exportService_1.ExportService.generateReportPDF(data, res);
        }
        catch (error) {
