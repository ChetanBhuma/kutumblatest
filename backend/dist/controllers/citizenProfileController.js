...
exports.CitizenProfileController = void 0;
...
class CitizenProfileController {
    ...
    static async updateProfile(req, res, next) {
        try {
            ...
            if (!citizenId && (req.user?.role === 'CITIZEN' || (req.user?.role).toUpperCase() === 'CITIZEN') && (req.user?.email || req.user?.mobileNumber)) {
                ...
                if (existing) {
                    ...
                    if (registration?.draftData) {
                        const draft = registration.draftData;
            ...
            if (updates.dateOfBirth) {
                ...
                if (isNaN(dob.getTime())) {
                    throw new errorHandler_1.AppError('Invalid Date of Birth', 400);
                }
            ...
            const citizen = await db.$transaction(async (tx) => {
                ...
                if (isAddressChanged) {
                    ...
                    logger_1.auditLogger.info('Address/Station change detected, resetting verification', {
                        citizenId,
                ...
                if (medicalHistory !== undefined) {
                    if (Array.isArray(medicalHistory)) {
                        ...
                        if (medicalHistory.length > 0) {
                            const validHistory = medicalHistory.filter((h) => h.conditionName?.trim());
            ...
            logger_1.auditLogger.info('Citizen updated profile with relations', {
                citizenId,
     ...
     */
    static async syncCitizenStatus(citizenId, citizenData) {
        try {
     ...
     * Request a visit
     */
    static async requestVisit(req, res, next) {
        try {
            const citizenId = req.user?.citizenId;
            if (!citizenId) {
                throw new errorHandler_1.AppError('Profile not found', 404);
            ...
            logger_1.auditLogger.info('Citizen requested visit', {
                citizenId,
                visitType
            });
     ...
     * Get own SOS alerts
     */
    static async getSOS(req, res, next) {
        try {
            const citizenId = req.user?.citizenId;
            if (!citizenId) {
                return res.json({
     ...
     * Create SOS alert
     */
    static async createSOS(req, res, next) {
        try {
            const citizenId = req.user?.citizenId;
            if (!citizenId) {
                throw new errorHandler_1.AppError('Profile not found', 404);
            ...
            if (!latitude || !longitude) {
                throw new errorHandler_1.AppError('Location is required', 400);
            }
            // Create SOS alert
            ...
            if (citizen) {
                ...
                const officers = citizen.Beat?.BeatOfficer?.map((o) => ({
                    name: o.name,
                    phone: o.mobileNumber
                })) || [];
                const contacts = citizen.EmergencyContact?.map((c) => ({
                    name: c.name,
                    phone: c.mobileNumber
                })) || [];
                // Send SOS notifications
                await notificationService_1.NotificationService.sendSOSAlert(citizen.fullName, citizen.mobileNumber, address || citizen.permanentAddress || 'Unknown location', latitude, longitude, { officers, emergencyContacts: contacts });
            }
            logger_1.auditLogger.error('SOS ALERT TRIGGERED BY CITIZEN', {
                alertId: sosAlert.id,
                citizenId,
                latitude,
                longitude,
    ...
    static async uploadDocument(req, res, next) {
        try {
            ...
            logger_1.auditLogger.info('Citizen uploaded document', {
                citizenId,
    ...
    static async updateNotifications(req, res, next) {
        try {
            ...
            const { allowNotifications, allowFamilyNotification, preferredContactMode, consentScheduledVisitReminder } = req.body;
    ...
    static async submitFeedback(req, res, next) {
        try {
            ...
            if (visitId) {
                ...
                logger_1.auditLogger.info('General feedback submitted by citizen', {
                    citizenId,
                    rating,
                    comments
                });
...
exports.CitizenProfileController = CitizenProfileController;
//# sourceMappingURL=citizenProfileController.js.map