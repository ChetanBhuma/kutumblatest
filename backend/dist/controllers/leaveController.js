...
class LeaveController {
    ...
    static async create(req, res, next) {
        try {
            ...
            if (overlapping) {
                throw new errorHandler_1.AppError('Leave request overlaps with existing leave', 409);
            }
            ...
            logger_1.auditLogger.info('Leave request created', {
                leaveId: leave.id,
                officerId,
                leaveType,
    ...
    static async list(req, res, next) {
        try {
            const { officerId, status, leaveType, startDate, endDate, page = 1, limit = 50 } = req.query;
            const where = {};
            if (officerId)
    ...
    static async approve(req, res, next) {
        try {
            ...
            if (leave.status !== 'Pending') {
                throw new errorHandler_1.AppError(`Cannot approve leave with status: ${leave.status}`, 400);
            }
            ...
            logger_1.auditLogger.info('Leave approved', {
                leaveId: id,
                officerId: leave.officerId,
    ...
    static async reject(req, res, next) {
        try {
            ...
            if (leave.status !== 'Pending') {
                throw new errorHandler_1.AppError(`Cannot reject leave with status: ${leave.status}`, 400);
            }
            const updated = await db.officerLeave.update({
                where: { id },
                data: {
            ...
            logger_1.auditLogger.warn('Leave rejected', {
                leaveId: id,
                officerId: leave.officerId,
    ...
    static async cancel(req, res, next) {
        try {
            ...
            if (!['Pending', 'Approved'].includes(leave.status)) {
                throw new errorHandler_1.AppError(`Cannot cancel leave with status: ${leave.status}`, 400);
            }
            const updated = await db.officerLeave.update({
                where: { id },
                data: { status: 'Cancelled' }
            });
            logger_1.auditLogger.info('Leave cancelled', {
                leaveId: id,
                officerId: leave.officerId,
     ...
     */
    static async getOfficerLeaves(req, res, next) {
        try {
