...
class OfficerController {
    ...
    static async list(req, res, next) {
        try {
            ...
            const result = await (0, pagination_1.paginatedQuery)(database_1.prisma.beatOfficer, {
    ...
    static async getById(req, res, next) {
        try {
            ...
            const completedVisits = await database_1.prisma.visit.count({
                where: {
                    officerId: id,
                    status: 'COMPLETED'
                }
            });
            const pendingVisits = await database_1.prisma.visit.count({
                where: {
                    officerId: id,
    ...
    static async create(req, res, next) {
        try {
            ...
            const existing = await database_1.prisma.beatOfficer.findFirst({
                where: {
                    OR: [
            ...
            if (existing) {
                throw new errorHandler_1.AppError('Officer with this badge number or mobile already exists', 409);
            }
            const officer = await database_1.prisma.beatOfficer.create({
                data: officerData,
                include: {
                    PoliceStation: true,
                    Beat: true
            ...
            logger_1.auditLogger.info('Officer created', {
    ...
    static async update(req, res, next) {
        try {
            ...
            const existing = await database_1.prisma.beatOfficer.findUnique({
                where: { id },
                include: { user: true }
            });
            if (!existing) {
            ...
            if (updateData.mobileNumber !== undefined)
                userUpdateData.phone = updateData.mobileNumber;
            // Transaction: Update BeatOfficer AND User (if needed)
            const officer = await database_1.prisma.$transaction(async (tx) => {
            ...
            logger_1.auditLogger.info('Officer updated', {
                officerId: officer.id,
                officerName: officer.name,
    ...
    static async delete(req, res, next) {
        try {
            const { id } = req.params;
            const officer = await database_1.prisma.beatOfficer.update({
                where: { id },
                data: { isActive: false }
            });
            logger_1.auditLogger.warn('Officer deleted', {
                officerId: officer.id,
                officerName: officer.name,
     ...
     */
    static async assignToBeat(req, res, next) {
        try {
            const { id } = req.params;
            ...
            let beat = null;
            if (beatId) {
                beat = await database_1.prisma.beat.findUnique({ where: { id: beatId } });
                if (!beat) {
            ...
            logger_1.auditLogger.info(beatId ? 'Officer assigned to beat' : 'Officer unassigned from beat', {
                officerId: officer.id,
                officerName: officer.name,
     ...
     */
    static async getWorkloadDistribution(req, res, next) {
        try {
            const where = (0, queryBuilder_1.buildWhereClause)(req.query, {
    ...
    static async getStatistics(req, res, next) {
        try {
            ...
            const [total, active, inactive, withBeat, withoutBeat] = await Promise.all([
                database_1.prisma.beatOfficer.count({ where }),
                database_1.prisma.beatOfficer.count({ where: { ...where, isActive: true } }),
                database_1.prisma.beatOfficer.count({ where: { ...where, isActive: false } }),
                database_1.prisma.beatOfficer.count({ where: { ...where, beatId: { not: null } } }),
                database_1.prisma.beatOfficer.count({ where: { ...where, beatId: null } })
            ]);
            res.json({
                success: true,
                data: {
     ...
     */
    static async transferOfficer(req, res, next) {
        try {
            const { id } = req.params;
            ...
            if (!officer.beatId) {
                throw new errorHandler_1.AppError('Officer has no current beat assignment', 400);
            }
            // Start transaction for atomic transfer
            const result = await database_1.prisma.$transaction(async (tx) => {
                const { OfficerTransferService } = await Promise.resolve().then(() => __importStar(require('../services/officerTransferService')));
                ...
                const reassignmentResults = await OfficerTransferService.reassignCitizensToNewOfficers(citizenIds, officer.beatId, id);
                // 3. Handle scheduled visits
                const visitResults = await OfficerTransferService.handleOfficerTransferVisits(officer.Visit, reassignmentResults);
            ...
            logger_1.auditLogger.info('Officer transferred successfully', {
                officerId: id,
                officerName: officer.name,
     ...
     */
    static async previewTransfer(req, res, next) {
        try {
            const { id } = req.params;
     ...
     */
    static async getTransferHistory(req, res, next) {
        try {
            const { id } = req.params;
            const history = await database_1.prisma.officerTransferHistory.findMany({
                where: { officerId: id },
...
//# sourceMappingURL=officerController.js.map