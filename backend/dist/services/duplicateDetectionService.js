...
exports.findAllDuplicates = exports.checkForDuplicates = void 0;
 ...
 */
const checkForDuplicates = async (citizenData) => {
    const duplicates = [];
    // 1. Exact Aadhaar match (highest priority)
    if (citizenData.aadhaarNumber) {
        const aadhaarMatches = await database_1.prisma.seniorCitizen.findMany({
            where: {
    ...
    const mobileMatches = await database_1.prisma.seniorCitizen.findMany({
        where: {
    ...
    mobileMatches.forEach(match => {
        ...
        if (duplicates.some(d => d.citizenId === match.id))
            return;
        const nameSimilarity = calculateSimilarity(normalizeName(citizenData.fullName), normalizeName(match.fullName));
        duplicates.push({
            citizenId: match.id,
            fullName: match.fullName,
            mobileNumber: match.mobileNumber,
    ...
    const allCitizens = await database_1.prisma.seniorCitizen.findMany({
        where: {
            isActive: true,
            ...(citizenData.excludeCitizenId ? { id: { not: citizenData.excludeCitizenId } } : {}),
    ...
    allCitizens.forEach(citizen => {
        ...
        if (duplicates.some(d => d.citizenId === citizen.id))
            return;
        const nameSimilarity = calculateSimilarity(normalizedInputName, normalizeName(citizen.fullName));
        // Only consider if name similarity is high
        if (nameSimilarity >= 85) {
            const reasons = [`Name similarity: ${nameSimilarity.toFixed(0)}%`];
...
const findAllDuplicates = async () => {
    ...
    for (const citizen of allCitizens) {
        const result = await (0, exports.checkForDuplicates)({
            fullName: citizen.fullName,
            mobileNumber: citizen.mobileNumber,
            aadhaarNumber: citizen.aadhaarNumber,
            dateOfBirth: citizen.dateOfBirth,
...
exports.findAllDuplicates = findAllDuplicates;
