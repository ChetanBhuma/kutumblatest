...
exports.cleanupStaleRegistrations = void 0;
 ...
 * Older than 24 hours.
 */
const cleanupStaleRegistrations = async () => {
    try {
        const cutoffDate = new Date(Date.now() - 24 * 60 * 60 * 1000); // 24 hours ago
        const staleCitizens = await database_1.prisma.seniorCitizen.findMany({
            where: {
                fullName: 'Unknown',
                createdAt: {
                    lt: cutoffDate
        ...
        for (const citizen of staleCitizens) {
            try {
                ...
                const activeVisits = await database_1.prisma.visit.count({
...
exports.cleanupStaleRegistrations = cleanupStaleRegistrations;
// Run if called directly
if (require.main === module) {
    (0, exports.cleanupStaleRegistrations)()
        .then(() => process.exit(0))
        .catch(() => process.exit(1));
}
//# sourceMappingURL=cleanupStaleRegistrations.js.map