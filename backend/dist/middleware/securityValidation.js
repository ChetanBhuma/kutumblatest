...
exports.preventNoSQLInjection = exports.detectMaliciousInput = void 0;
...
const detectMaliciousInput = (req, res, next) => {
    const checkInput = (input, location) => {
        if (typeof input === 'string') {
            // Check for SQL injection
            for (const pattern of SQL_INJECTION_PATTERNS) {
                if (pattern.test(input)) {
                    logger_1.auditLogger.warn('SQL injection attempt detected', {
                        location,
                        input,
                        ip: req.ip,
            ...
            for (const pattern of XSS_PATTERNS) {
                if (pattern.test(input)) {
                    logger_1.auditLogger.warn('XSS attempt detected', {
                        location,
                        input,
                        ip: req.ip,
                        userAgent: req.get('user-agent'),
            ...
            for (const pattern of PATH_TRAVERSAL_PATTERNS) {
                if (pattern.test(input)) {
                    logger_1.auditLogger.warn('Path traversal attempt detected', {
                        location,
                        input,
                        ip: req.ip,
                        userAgent: req.get('user-agent'),
 ...
 */
const preventNoSQLInjection = (req, _res, next) => {
    const sanitize = (obj) => {
        if (obj && typeof obj === 'object') {
            for (const key in obj) {
                ...
                if (key.startsWith('$')) {
                    delete obj[key];
                    logger_1.auditLogger.warn('NoSQL injection attempt detected', {
                        key,
                        ip: req.ip,
                        path: req.path
                    });
