...
async function verifyUserCRUD() {
    ...
    try {
        // --- PRE-CONFIGURATION ---
        // Ensure roles exist
        const officerRole = await database_1.prisma.role.findUnique({ where: { code: roleCode } });
        if (!officerRole) {
            console.log(`⚠️ ${roleCode} role not found. Creating it...`);
            await database_1.prisma.role.create({ data: { code: roleCode, name: 'Officer', permissions: [], isActive: true } });
        }
        ...
        try {
            const createRes = await axios_1.default.post(`${API_URL}/users`, {
            ...
            if (createRes.data.data.role !== roleCode)
                throw new Error(`Role mismatch. Expected ${roleCode}, got ${createRes.data.data.role}`);
        }
        catch (error) {
        ...
        const listRes = await axios_1.default.get(`${API_URL}/users?search=${targetUserEmail}`, { headers: { Authorization: `Bearer ${adminToken}` } });
        ...
        try {
            const updateRes = await axios_1.default.put(`${API_URL}/users/${targetUserId}/role`, { roleCode: newRoleCode }, { headers: { Authorization: `Bearer ${adminToken}` } });
            if (updateRes.data.data.role !== newRoleCode)
                throw new Error('Role update failed in response');
            // Verify in DB/List
            const verifyRes = await axios_1.default.get(`${API_URL}/users?search=${targetUserEmail}`, { headers: { Authorization: `Bearer ${adminToken}` } });
            const verifiedUser = verifyRes.data.data.users.find((u) => u.id === targetUserId);
            if (verifiedUser.role !== newRoleCode)
                throw new Error('Role update not reflected in list');
        ...
        // --- CLEANUP ---
        console.log('\n🧹 Final Cleanup...');
        await database_1.prisma.user.deleteMany({ where: { email: { in: [adminEmail, targetUserEmail] } } });
        console.log('✅ Cleanup complete.');
        process.exit();
    }
}
verifyUserCRUD();
//# sourceMappingURL=verify-user-crud.js.map