...
async function verifyRoleCRUD() {
    ...
    try {
        ...
        if (createRes.data.data.code !== testRoleCode) throw new Error('Role Code mismatch');
        ...
        const readRes = await axios.get(
            `${API_URL}/roles/${createdRoleId}`,
            { headers: { Authorization: `Bearer ${adminToken}` } }
        );
        console.log('Role Fetched.');
        if (readRes.data.data.name !== 'Test CRUD Role') throw new Error('Role Name mismatch');
        ...
        if (updateRes.data.data.name !== 'Updated Role Name') throw new Error('Update failed');
        if (!updateRes.data.data.permissions.includes('citizens.write')) throw new Error('Permissions update failed');
        ...
        await prisma.user.deleteMany({ where: { email: adminEmail } });
        if (createdRoleId) {
            try { await prisma.role.delete({ where: { id: createdRoleId } }); } catch (e) {}
        }
