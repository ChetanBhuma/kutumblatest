...
import { InteractiveDeviceMockups } from '@/components/landing/InteractiveDeviceMockups'
import { InteractiveSosSimulator } from '@/components/landing/InteractiveSosSimulator'
import { WelfareVisitExplorer } from '@/components/landing/WelfareVisitExplorer'
import { InteractiveBeatFinder } from '@/components/landing/InteractiveBeatFinder'
...
export default function LandingPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-[#0F52BA] selection:text-white">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <nav className="flex items-center gap-3" aria-label="Main Navigation">
                            ...
                            <Link href="/admin/login" aria-label="Login to Admin & Officer Portal">
                                <Button className="bg-gradient-to-r from-[#0F52BA] to-[#720924] hover:from-[#0F52BA]/90 hover:to-[#720924]/90 text-white font-bold transition-all duration-300 shadow-[0_0_15px_rgba(15,82,186,0.5)]">
                                    Officer Portal
                                </Button>
                            </Link>
                        </nav>
            ...
            <section className="relative w-full h-[360px] md:h-[480px] lg:h-[540px] overflow-hidden bg-slate-950 group" aria-label="Banner Slider">
                {/* Slider Images */}
                ...
                <button
                    onClick={prevSlide}
                    className="absolute left-4 top-1/2 -translate-y-1/2 bg-slate-950/60 hover:bg-[#0F52BA] text-white p-3 rounded-full transition-all duration-300 opacity-0 group-hover:opacity-100 z-20 border border-white/20 shadow-xl backdrop-blur-sm"
                ...
                <button
                    onClick={nextSlide}
                    className="absolute right-4 top-1/2 -translate-y-1/2 bg-slate-950/60 hover:bg-[#0F52BA] text-white p-3 rounded-full transition-all duration-300 opacity-0 group-hover:opacity-100 z-20 border border-white/20 shadow-xl backdrop-blur-sm"
                ...
                <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex gap-2 z-20 bg-slate-950/40 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">
                    {slides.map((_, index) => (
                        <button
                            key={index}
            ...
            <section className="py-14 bg-gradient-to-r from-[#061224] via-[#0F3274] to-[#720924] text-white relative overflow-hidden border-y border-white/10">
                <div className="container mx-auto px-4 relative z-10">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
                        <div className="group p-4 rounded-xl bg-slate-950/40 backdrop-blur-sm border border-white/10 hover:border-[#D4AF37]/50 transition-all">
                            <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-blue-500/20 text-blue-300 mb-2 group-hover:scale-110 transition-transform">
                                <Users className="w-6 h-6" />
                            </div>
                            <div className="text-3xl md:text-4xl font-black text-white">{stats.citizens.toLocaleString()}+</div>
                            <p className="text-xs text-blue-200 mt-1 font-medium">Registered Senior Citizens</p>
                        </div>

                        <div className="group p-4 rounded-xl bg-slate-950/40 backdrop-blur-sm border border-white/10 hover:border-[#D4AF37]/50 transition-all">
                            <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-rose-500/20 text-rose-300 mb-2 group-hover:scale-110 transition-transform">
                                <MapPin className="w-6 h-6" />
                            </div>
                            <div className="text-3xl md:text-4xl font-black text-white">{stats.stations} Police</div>
                            <p className="text-xs text-blue-200 mt-1 font-medium">Districts & Sub-Divisions</p>
                        </div>

                        <div className="group p-4 rounded-xl bg-slate-950/40 backdrop-blur-sm border border-white/10 hover:border-[#D4AF37]/50 transition-all">
                            <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-amber-500/20 text-[#D4AF37] mb-2 group-hover:scale-110 transition-transform">
                                <Heart className="w-6 h-6" />
                            </div>
                            <div className="text-3xl md:text-4xl font-black text-white">{stats.visits.toLocaleString()}+</div>
                            <p className="text-xs text-blue-200 mt-1 font-medium">Monthly Physical Visits</p>
                        </div>

                        <div className="group p-4 rounded-xl bg-slate-950/40 backdrop-blur-sm border border-white/10 hover:border-[#D4AF37]/50 transition-all">
                            <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-emerald-500/20 text-emerald-300 mb-2 group-hover:scale-110 transition-transform">
                                <Clock className="w-6 h-6" />
                            </div>
                            <div className="text-3xl md:text-4xl font-black text-white">&lt; {stats.response} Mins</div>
                            <p className="text-xs text-blue-200 mt-1 font-medium">Emergency Response Time</p>
                        </div>
                    </div>
                </div>
            </section>

            {/* 5. Section: Interactive Product Mockup Tabs (Citizen App, Beat Tablet, Command Center, Digital ID) */}
            <div id="interactive-features">
                <InteractiveDeviceMockups />
            </div>

            {/* 6. Section: Interactive Emergency SOS Simulator */}
            <InteractiveSosSimulator />

            {/* 7. Section: Welfare Visit Explorer & Vulnerability Index Calculator */}
            <WelfareVisitExplorer />

            {/* 8. Section: Interactive Beat Officer & Station Finder */}
            <InteractiveBeatFinder />

            {/* 9. Section: Community Trust, Testimonials & Cyber Advisory */}
            <TrustAndAdvisorySection />

            {/* 10. Call to Action Section with Police Insignia */}
            <section className="py-20 bg-gradient-to-r from-[#061224] via-[#0F3274] to-[#720924] text-white relative overflow-hidden border-t border-white/15">
                <div className="container mx-auto px-4 text-center relative z-10 max-w-4xl">
                    <Badge className="bg-[#D4AF37] text-slate-950 font-bold mb-4">
                    ...
                    <div className="flex gap-4 justify-center flex-wrap">
                        <Link href="/citizen-portal/register">
                            <Button size="lg" className="bg-white text-slate-950 hover:bg-slate-100 font-extrabold text-base px-8 py-6 rounded-xl shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                                <Users className="mr-2 h-5 w-5 text-[#0F52BA]" />
                                Register Now - It's Free
                                <ArrowRight className="ml-2 h-5 w-5 text-[#720924]" />
                            </Button>
                        </Link>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
                        ...
                        <div>
                            ...
                            <ul className="space-y-2.5 text-xs text-slate-400">
                                ...
                                <li><Link href="#interactive-features" className="hover:text-white transition-colors">Platform Features & Mockups</Link></li>
                            </ul>
                        </div>
